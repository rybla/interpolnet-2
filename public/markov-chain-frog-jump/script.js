...
const colors = {
  ...
  lilypadHighlight:
    style.getPropertyValue("--lilypad-highlight").trim() || "#4ceb84",
  ...
  arrowHighlight:
    style.getPropertyValue("--arrow-highlight").trim() ||
    "rgba(255, 235, 59, 0.9)",
...
function resize() {
  ...
  statePositions = [
    ...
    {
      x: cx + xOffset + radius * Math.cos(Math.PI / 6),
  ...
  for (let from = 0; from < NUM_STATES; from++) {
    ...
    for (let to = 0; to < NUM_STATES; to++) {
      ...
      slider.addEventListener("input", (e) =>
        handleSliderChange(from, to, parseFloat(e.target.value)),
      );
...
function drawArrow(x1, y1, x2, y2, isSelfLoop, weight, isHighlighted) {
  ...
  if (isSelfLoop) {
    ...
    ctx.lineTo(
      endX - headlen * Math.cos(tangentAngle - Math.PI / 6),
      endY - headlen * Math.sin(tangentAngle - Math.PI / 6),
    );
    ctx.lineTo(
      endX - headlen * Math.cos(tangentAngle + Math.PI / 6),
      endY - headlen * Math.sin(tangentAngle + Math.PI / 6),
...
function draw(timestamp) {
  ...
  for (let i = 0; i < NUM_STATES; i++) {
    ...
    const isCurrent =
      (i === currentState && !isJumping) ||
      (isJumping && i === jumpTo && jumpProgress > 0.8);
  ...
  if (isJumping) {
    ...
    ctx.ellipse(
      0,
      30,
      FROG_RADIUS * shadowScale,
      FROG_RADIUS * 0.4 * shadowScale,
      0,
      0,
      Math.PI * 2,
    );
